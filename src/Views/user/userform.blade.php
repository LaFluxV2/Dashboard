...
@section('content-area')
    <div class="right_col" role="main">
        ...
        ?>
        <div class="x_panel">
            {!!Form::open(array('route' => $action, 'method' => 'post', 'onsubmit'=>'submitCheck();'))!!}
            <div class="row">
                ...
                {{--@if(\Auth::user()->groups == 1)--}}
                    {{--<div class="col-xs-12 col-sm-12 col-md-3 col-lg-3">--}}
                        {{--<div class="form-group {{ $errors->has('groups') ? 'has-error' : '' }} control-required">--}}
                            ...
                            {{--{!! Form::select('groups', array('0'=>'---Select---','1'=>'Super Admin','2'=>'Admin')+$groups->toArray(), isset($user->groups) ? $user->groups :null, [--}}
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            jQuery('.select_firm').hide();
            jQuery('.select_dept').hide();

            jQuery(document).on('change', '#groups', function () {
                if (jQuery('#groups').val() == 1) {
                    jQuery('.select_firm').hide();
                    jQuery('.select_dept').hide();
                }
                else if (jQuery('#groups').val() == 2) {
                    jQuery('.select_firm').show();
                    jQuery('.select_dept').hide();
                }
                else if (jQuery('#groups').val() > 2) {
                    jQuery('.select_firm').hide();
                    jQuery('.select_dept').show();
                }
                else {
